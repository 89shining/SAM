...
class PolyLRScheduler:
    def __init__(self, optimizer, max_steps, initial_lr=1e-4, exponent=0.9):
    ...
    def step(self):
        lr = self.initial_lr * (1 - self.current_step / self.max_steps) ** self.exponent
        lr = max(lr, 1e-8)  # 防止最后学习率太小直接为0，夹一下
...
def train_net(net,
    ...
    # 总步数 = epoch数 × 每epoch的batch数
    max_steps = epochs * len(train_loader)
    # 自己定义PolyLR调度器
    scheduler = PolyLRScheduler(optimizer, max_steps=max_steps, initial_lr=lr, exponent=0.9)
...
if __name__ == '__main__':
    # 配置日志保存
    saveDir = "C:/Users/dell/Desktop/task/finetune"
    # 自动创建目录
    ...
    if missing_keys:
        logging.warning(f"[Warning] Missing keys :\n{missing_keys}")
        # print(f"[Warning] Missing keys :\n{missing_keys}")
    if unexpected_keys:
        logging.warning(f"[Warning] Unexpected keys in checkpoint:\n{unexpected_keys}")
