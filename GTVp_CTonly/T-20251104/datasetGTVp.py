...
class SAMDataset(Dataset):
    def __init__(self, csv_path, root_dir, nii_dir, target_size):
        ...
        # 缓存每个患者的 spacing，避免重复读取 .nii.gz
        self.spacing_cache = {}
        self._cache_spacing()

    def _cache_spacing(self):
        """提前读取每个患者的 NIfTI spacing 信息"""
    ...
    def __getitem__(self, idx):
        ...
        if patient_id not in self.spacing_cache:
            raise FileNotFoundError(f"Spacing not cached for {patient_id}")
        spacing_x, spacing_y = self.spacing_cache[patient_id]

        # 生成box提示
        has_gt = float(resized_mask.sum() > 0)
        box_train = self.get_box_train(resized_mask, spacing_x, spacing_y)
        box_val = self.get_box_val(resized_mask, spacing_x, spacing_y)
