"""
第一个外扩值 -> 对所有患者第一次随机框推理生成png -> 合成nii 计算3d dice-> 第2次随机框推理....
第2个外扩值......
......
"""

"""
最终版：保留5折融合 + 原始Excel写法(overview+每个病人sheet) 
+ 缓存image_embeddings加速 + 不计算HD95 + 保留2位小数
+ 修正3D Dice（保持与原始NIfTI一致：rot90+flip）
+ tqdm进度条
"""
...
base_output_dir = "/home/wusi/SAMdata/20250821/testresults/max_expand_cm"
max_expand_cm_list = [0.5, 1, 1.5, 2.0]
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ========= 每例重复次数 =========
n_repeat = 10  # 可调小，正式跑再设大
...
for max_expand_cm in max_expand_cm_list:
    ...
    # ===== 写 Excel =====
    xlsx_path = os.path.join(output_dir, f"r_summary_expand_{max_expand_cm}cm.xlsx")
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        ...
        if df_all:
            ...
            overview[["dice3d","dL_mean_mm","dR_mean_mm","dT_mean_mm","dB_mean_mm"]] = \
                overview[["dice3d","dL_mean_mm","dR_mean_mm","dT_mean_mm","dB_mean_mm"]].round(2)
        ...
        for pa, rows in sorted(patient_rows.items(), key=lambda x: x[0]):
            ...
            df[["dice3d","dL_mean_mm","dR_mean_mm","dT_mean_mm","dB_mean_mm"]] = \
                df[["dice3d","dL_mean_mm","dR_mean_mm","dT_mean_mm","dB_mean_mm"]].round(2)
