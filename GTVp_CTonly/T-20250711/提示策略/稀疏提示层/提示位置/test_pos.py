...
from testdatasetGTVp_pos import TestDataset
import shutil
...
base_output_dir = "/home/wusi/SAMdata/20251104_GTVp/TestResults/Pos_box_prompts" # 预测输出结果根目录
expand_cm_list = [0.5]  # 外扩距离（单位：cm）
prompt_mode_list = ["middle_z", "max_area", "mid_volume", "random"]   # 四种提示层模式
...
for expand_cm in expand_cm_list:
    ...
    for prompt_mode in prompt_mode_list:
        ...
        if prompt_mode == "random":
            random.seed(42)  # 固定随机种子，确保每次运行 repeat_list 一致
            # 随机偏移次数
            i = 10  # 想要生成几次偏移
            repeat_list = random.sample(range(1000), i)
            print(f"本次repeat_list={repeat_list}")

        else:
            repeat_list = [0]  # 其余只跑一次

        for repeat_id in repeat_list:
            print(f"\n=== 模式={prompt_mode}, 随机重复={repeat_id} ===")

            output_dir = os.path.join(base_output_dir, f"{prompt_mode}_rep{repeat_id}")
