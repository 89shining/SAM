...
def train_one_fold(fold, train_idx, val_idx, all_image_paths, dataset, net, device,
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader), desc=f'[Train Fold {fold+1}]', unit='batch', disable=True) as pbar:
            # 传入一个batch
            for image, mask, point_coords, point_labels in train_loader:
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader), desc=f'[Train Fold {fold+1}]', unit='batch', disable=True) as pbar:
                # 传入一个batch
                for image, mask, point_coords, point_labels in val_loader:
                    imgs = image.to(device).float()
                    true_masks = mask.to(device).float()
                    points_coords = point_coords.to(device).float()
                    points_labels = point_labels.to(device).float()
...
if __name__ == '__main__':
    root_dir = '/home/intern/wusi/Dataset/GTVnd/GTVndtrain_dataset'   # traindataset的目录
    csv_path = '/home/intern/wusi/Dataset/GTVnd/GTVndtrain_dataset/train_tiff.csv'
    save_dir = '/home/intern/wusi/Dataset/GTVnd/trainresults_kfold'
    os.makedirs(save_dir, exist_ok=True)

    # Logging setup
    log_path = os.path.join(save_dir, 'logs', 'train.log')
    ...
    dataset = SAMDataset(csv_path=csv_path, root_dir=root_dir, target_size=(1024, 1024), num_pos=5, num_neg=5)
    all_image_paths = pd.read_csv(csv_path, header=None, names=["image", "mask"])["image"].tolist()

    sam_checkpoint = "/home/intern/wusi/segment-anything/demo/configs/checkpoint/sam_vit_h_4b8939.pth"
    model_type = "vit_h"
    # 训练设备：gpu 0
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        train_one_fold(fold, train_idx, val_idx, all_image_paths, dataset, net, device,
                       epochs=200, batch_size=2, lr=0.0001, save_dir=save_dir)
