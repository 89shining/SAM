# Five-fold cross-validation training for CTV (nnUNet box prompt)
import json
import os
import sys
...
def _forward_masks_batched(net, imgs, boxes, freeze_image_encoder=True):
    """
    Keep batched image encoder for speed, but run prompt+decoder per sample
    to stay compatible with this SAM implementation.
    """
    ...
    for epoch in range(start_epoch, epochs):
        ...
        with tqdm(total=len(train_loader), desc=f'[Train Fold {fold + 1}]', unit='batch', disable=True) as pbar:
            for batch in train_loader:
                ...
                masks_pred = _forward_masks_batched(net, imgs, boxes, freeze_image_encoder=True)
