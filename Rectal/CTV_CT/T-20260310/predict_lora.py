...
import shutil
from Mycodes.sam_lora import inject_lora_to_sam_image_encoder

# ========= 配置路径（请根据实际路径修改） =========
fold_ckpts = [
    "/home/wusi/SAMdata/Rectal/20260310_CTV/dataset/trainresult_3slices/freeze_img_lora/fold_1/weights/best.pth",                      # 每个fold的best权重路径
...
model_type = "vit_b"
csv_path = "/home/wusi/SAMdata/Rectal/20260310_CTV/dataset/test/test_pseudo_rgb.csv"   # 测试数据CSV文件路径
root_dir = "/home/wusi/SAMdata/Rectal/20260310_CTV/dataset/test"                         # 测试集根目录
image_dir = "/home/wusi/SAMdata/Rectal/20260310_CTV/dataset/test/pseudo_rgb_images"      # 测试image
ii_dir = "/home/wusi/SAMdata/Rectal/20260310_CTV/datanii/test_nii"                      # 对应的参考NIfTI图像路径（含image.nii.gz）
base_output_dir = "/home/wusi/SAMdata/Rectal/20260310_CTV/dataset/testresults_3slices/freeze_img_lora" # 预测输出结果根目录
expand_pixel_list = [3]  # 不同外扩像素值

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

for expand_pixel in expand_pixel_list:
    print(f"\n=== 正在处理外扩像素: {expand_pixel} ===")
    output_dir = os.path.join(base_output_dir, f"expand_{expand_pixel}p")
    os.makedirs(output_dir, exist_ok=True)
    tmp_png_dir = os.path.join(output_dir, "tmp_png")
    os.makedirs(tmp_png_dir, exist_ok=True)

    # ========= 数据加载 =========
    test_dataset = TestDataset(csv_path=csv_path, root_dir=root_dir, nii_dir=ii_dir, target_size=(1024, 1024), expand_pixel=expand_pixel)
    ...
    for ckpt in fold_ckpts:
        ...
        # 再注入和训练时一致的 LoRA 结构
        model = inject_lora_to_sam_image_encoder(model, r=4, alpha=8)

        # 再加载该 fold 的训练权重，并检查是否匹配
        msg = model.load_state_dict(torch.load(ckpt, map_location=device), strict=False)
        print(f"\n[Load LoRA ckpt] {ckpt}")
        print("Missing keys:", msg.missing_keys[:10], "..." if len(msg.missing_keys) > 10 else "")
        print("Unexpected keys:", msg.unexpected_keys[:10], "..." if len(msg.unexpected_keys) > 10 else "")
    ...
    print(f"外扩 {expand_pixel}px 融合预测完成，结果保存在: {output_dir}")
