"""
SAM image-encoder prior experiments using nnUNet probability map.
MODE=modulation
- modulation: ch1=CT*P, ch2=CT*P, ch3=CT*P
- injection:  ch1=CT, ch2=P(0-255), ch3=CT
"""
...
class SAMImageEncoderDatasetFromNiiGz(Dataset):
    """
    Returns:
      image: float32 [3, 1024, 1024], 0-255
      GT: float32 [1, H, W] (original size)
      mask_prompt: float32 [1, 256, 256] (probability logit for prompt encoder)
    """
    ...
    def __init__(
        ...
        if len(self.index) == 0:
            raise RuntimeError("No valid GT slices found.")
