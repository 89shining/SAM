# Five-fold cross-validation training for CTV (nnUNet probability -> image encoder prior)
import argparse
import json
import os
import sys
...
from dataset import SAMImageEncoderDatasetFromNiiGz
...
def _forward_masks_batched(net, imgs, mask_inputs, freeze_image_encoder=False):
    ...
    try:
        ...
        if "unable to find an engine" not in str(e).lower():
            raise
        logging.warning("cuDNN engine find failed in image_encoder; retry with cuDNN disabled for this step.")
...
def train_one_fold(fold, train_idx, val_idx, dataset, net, device, epochs, batch_size, lr, save_dir):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        with tqdm(total=len(train_loader), desc=f"[Train Fold {fold + 1}]", unit="batch", disable=True) as pbar:
            for batch in train_loader:
                ...
                masks_pred = _forward_masks_batched(net, imgs, mask_inputs, freeze_image_encoder=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train SAM image encoder prior model.")
    parser.add_argument(
        "--fold",
        type=int,
        default=None,
        help="Fold index to run (0-based). Default: run all 5 folds.",
    )
    ...
    nnunet_prompt_npz_dir = "/home/wusi/nnUNet/nnUNetFrame/DATASET/nnUNet_results/Dataset014_RectalCTV60pCrop/nnUNetTrainer__nnUNetPlans__3d_fullres/testResult_5folds_probability"
    save_dir = "/home/wusi/segment-anything/SAMdata/Rectal/20260401_CTV/nnunet_probability/Image_encoder/singleChannel_injection/TrainResult"
    os.makedirs(save_dir, exist_ok=True)

    dataset = SAMImageEncoderDatasetFromNiiGz(
    ...
    if device.type == "cuda":
        ...
        print(
            f"[CUDA] logical_device_count={torch.cuda.device_count()}, "
    ...
    if args.fold is not None and (args.fold < 0 or args.fold >= n_splits):
        raise ValueError(f"--fold must be in [0, {n_splits - 1}], got {args.fold}")

    if args.fold is None:
        print(f"[Train] mode={mode}, run all {n_splits} folds.")
    else:
        print(f"[Train] mode={mode}, run only fold_{args.fold + 1}.")
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(np.zeros(len(dataset)), groups=groups)):
        ...
        logging.info(f"Image prior mode: {mode}")
        ...
        train_one_fold(fold, train_idx, val_idx, dataset, net, device, epochs=200, batch_size=8, lr=0.0001, save_dir=save_dir)
