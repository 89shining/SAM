...
def train_one_fold(fold, train_idx, val_idx, all_image_paths, dataset, net, device,
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader), desc=f'[Train Fold {fold + 1}]', unit='batch', disable=True) as pbar:
            # 传入一个batch
            for image, mask, box_train, _ in train_loader:
                imgs = image.to(device).float()
                true_masks = mask.to(device).float()
                bbox = box_train.to(device).float()
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader), desc=f'[Train Fold {fold + 1}]', unit='batch', disable=True) as pbar:
                # 传入一个batch
                for image, mask, box_train, box_val in val_loader:
                    imgs = image.to(device).float()
                    true_masks = mask.to(device).float()
                    bbox = box_val.to(device).float()
...
if __name__ == '__main__':
    root_dir = '/home/intern/wusi/Dataset/GTVp0526/newdataset'  # traindataset的目录
    csv_path = '/home/intern/wusi/Dataset/GTVp0526/newdataset/train_tiff.csv'
    nii_dir = "/home/intern/wusi/Dataset/GTVp0526/traindatanii"   # trainnii数据文件夹
    save_dir = '/home/intern/wusi/Dataset/GTVp0526/trainresults_kfold_new'    # 训练结果保存文件夹
