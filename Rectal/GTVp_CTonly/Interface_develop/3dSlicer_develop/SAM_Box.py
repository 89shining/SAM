...
def parse_args():
    parser = argparse.ArgumentParser(description="Clinical SAM GTVp segmentation with arbitrary multi-box prompts")
    parser.add_argument(
        "-nii", "--niiPath",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="多个提示框boxes.json文件的完整路径（至少1个，不同提示框需位于不同切片）",
    )
    parser.add_argument(
        "-out", "--outputPath",
        type=str,
...
def load_boxes(json_path: Path, volume_shape):
    ...
    for i, box in enumerate(boxes, start=1):
        ...
        if not (0 <= z < Z):
            raise ValueError(
                f"第{i}个box的slice_index={z}超出CT范围0~{Z - 1}。"
            )
        if not (0 <= x0 < x1 < W):
            raise ValueError(
                f"第{i}个box的x坐标不合法：x0={x0}, x1={x1}, width={W}。"
            )
        if not (0 <= y0 < y1 < H):
            raise ValueError(
                f"第{i}个box的y坐标不合法：y0={y0}, y1={y1}, height={H}。"
            )
    ...
    slice_indices = [b["slice_index"] for b in cleaned]
    if len(set(slice_indices)) != len(slice_indices):
        raise ValueError("不同提示框必须位于不同的切片，不能在同一slice重复设置box。")
...
def get_box_for_slice(boxes, z):
    """
    根据任意数量的anchor boxes，为目标slice返回box。

    - 1个box：仅该提示层有效；
    - >=2个box：在每一对相邻anchor之间做分段线性插值；
    - 不在最上/最下anchor范围之外做外推。
    """
    if not boxes:
        raise ValueError("boxes不能为空。")

    # 单个box：只预测这一层，不做插值。
    if len(boxes) == 1:
        only_box = boxes[0]
        if z != only_box["slice_index"]:
            raise ValueError(
                f"仅有1个提示框时，只允许预测slice {only_box['slice_index']}，当前slice={z}。"
            )
        return np.array(
            [
                only_box["x0"],
    ...
    z_first = boxes[0]["slice_index"]
    z_last = boxes[-1]["slice_index"]

    if not (z_first <= z <= z_last):
        raise ValueError(
            f"slice {z}不在提示框范围{z_first}~{z_last}内。"
        )

    # 如果恰好是某个anchor层，直接返回该anchor box。
    for box in boxes:
        if z == box["slice_index"]:
            return np.array(
                [box["x0"], box["y0"], box["x1"], box["y1"]],
                dtype=np.float32,
            )

    # 找到包围当前z的相邻两个anchor，做线性插值。
    for left_box, right_box in zip(boxes[:-1], boxes[1:]):
        zl = left_box["slice_index"]
        zr = right_box["slice_index"]
        if zl < z < zr:
            return interpolate_two_boxes(left_box, right_box, z)

    raise RuntimeError(
        f"未能为slice {z}找到相邻提示框，请检查boxes排序和slice_index。"
    )
...
def main():
    ...
    print(f"\n读取到 {len(boxes)} 个医生提示框：")
    for i, box in enumerate(boxes, start=1):
        print(
            f"  Box {i}: slice {box['slice_index']}, "
    ...
    total_slices = z_last - z_first + 1

    print("\n==========================================")
    print(
        f"开始预测：slice {z_first}~{z_last}，"
        f"共 {total_slices} 层，{len(boxes)} 个anchor box"
    )
    print("==========================================")

    for count, z in enumerate(range(z_first, z_last + 1), start=1):
