...
DEFAULT_OUTPUT_ROOT = "/home/wusi/segment-anything/SAMdata/Rectal/20251224_GTVp/TestResults/Prompt_variability"
...
def parse_args():
    ...
    parser.add_argument("--upper_limits_cm", nargs="+", type=float, default=[0.5, 1.0, 1.5])
...
def build_trial_boxes_and_prompt_records(
    ...
    middle_z = int(rng.choice(valid_z[1:-1]))
    prompt_z_by_type = {
        "superior": superior_z,
        "middle": middle_z,
        "inferior": inferior_z,
...
def asd_binary(gt, pred, spacing):
    gt = gt > 0
    pred = pred > 0
    if not gt.any() and not pred.any():
        return 0.0
...
def compute_all_metrics(pred_vol, gt_vol, spacing):
    ...
    return {
        ...
        "ASD_mm": asd_binary(gt, pred, spacing),
    ...
    for upper_idx, upper_limit_cm in enumerate(args.upper_limits_cm):
        ...
        for patient_id in tqdm(sorted(rows_by_patient.keys()), desc=f"upper {fmt_cm(upper_limit_cm)}cm"):
            ...
            for trial in range(1, args.n_trials + 1):
                seed = args.seed + upper_idx * 1_000_000 + trial * 10_000 + stable_int(patient_id)
