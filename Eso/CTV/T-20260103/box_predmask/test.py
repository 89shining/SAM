...
model_type = "vit_b"
csv_path = "/home/wusi/SAMdata/Eso/20251217_CTV/dataset/test/test_rgb.csv"   # 测试数据CSV文件路径
root_dir = "/home/wusi/SAMdata/Eso/20251217_CTV/dataset/test"              # 测试集dataset根目录
image_dir = "/home/wusi/SAMdata/Eso/20251217_CTV/dataset/test/rgb_images"   # 测试image
ii_dir = "/home/wusi/SAMdata//Eso/20251217_CTV/datanii/test_nii"          # 对应的参考NIfTI图像路径（含image.nii.gz）
base_output_dir = "/home/wusi/SAMdata/Eso/20260104_CTV/box_predmask/TestResult" # 预测输出结果根目录
    ...
    with torch.no_grad():
        for idx, (image, mask, box, original_size, image_path) in enumerate(test_loader):
            ...
            for net in nets:
                ...
                sparse_embeddings_1, dense_embeddings_1 = net.prompt_encoder(points=None, boxes=bbox, masks=None)
                low_res_masks_1, _ = net.mask_decoder(
                ...
                low_res_masks_1 = low_res_masks_1.detach()

                sparse_embeddings_2, dense_embeddings_2 = net.prompt_encoder(
                    points=None,
                    boxes=bbox,
                    masks=low_res_masks_1)

                low_res_masks_2, _ = net.mask_decoder(
                    image_embeddings=image_embeddings,
                    image_pe=net.prompt_encoder.get_dense_pe(),
                    sparse_prompt_embeddings=sparse_embeddings_2,
                    dense_prompt_embeddings=dense_embeddings_2,
                    multimask_output=False)

                masks = net.postprocess_masks(low_res_masks_2, input_size=imgs.shape[-2:], original_size=original_size)
