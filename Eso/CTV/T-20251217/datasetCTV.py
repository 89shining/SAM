...
def visualize_image_mask_box(image, mask, box, save_path=None, alpha=0.5):
    """
    image: (3, H, W) Tensor  -> 1024
    mask:  (1, h, w) Tensor  -> 512
    box:   (4,) Tensor       -> 1024 坐标
    """

    image_np = image.permute(1, 2, 0).cpu().numpy()  # (1024,1024,3)
    mask_np = mask.squeeze(0).cpu().numpy()          # (512,512)
...
if __name__ == '__main__':
    dataset = SAMDataset(
        csv_path=r"C:\Users\dell\Desktop\Eso-CTV\20251217\dataset/train/train_rgb.csv",
        root_dir=r"C:\Users\dell\Desktop\Eso-CTV\20251217\dataset\train",    # train数据文件夹
        nii_dir=r"C:\Users\dell\Desktop\Eso-CTV\20251217\datanii/train_nii",   # trainnii数据文件夹
    ...
    for batch_idx, batch in enumerate(dataloader):
        image = batch['image']
        mask = batch['GT']
        box_train = batch['train_box']
        box_val = batch['val_box']

        visualize_image_mask_box(
            image=image[0],
            mask=mask[0],  # 512×512
            box=box_train[0].squeeze(0),  # 1024 坐标
            save_path=f"./vis_train_{batch_idx}.png"
        )
